import { degreesLat, degreesLong, degreesToRadians, ecfToLookAngles, eciToEcf, eciToGeodetic, gstime, propagate, } from "satellite.js";
const toDegrees = (radians) => (radians * 180) / Math.PI;
export function computeSnapshot(satellites, observer, at = new Date()) {
    const visible = [];
    let propagated = 0;
...
export function lookAt(satellite, observer, at) {
    const state = propagate(satellite.satrec, at);
    if (!state || typeof state.position === "boolean")
        return null;
...
export function predictPasses(satellites, observer, start = new Date(), options = {}) {
    ...
    for (let timeMs = start.getTime(); timeMs <= endMs; timeMs += stepSeconds * 1000) {
        ...
        for (const satellite of satellites) {
            ...
            if (look && look.elevation > 0) {
                ...
                if (current.peakElevation >= minimumPeak)
                    completed.push(current);
            }
        }
    }
    for (const pass of active.values()) {
        if (pass.peakElevation >= minimumPeak)
            completed.push(pass);
    }
    return completed.sort((a, b) => a.peakAt.getTime() - b.peakAt.getTime());
}
export function visibilitySeries(satellites, observer, start = new Date()) {
    const series = [];
    for (let minute = 0; minute <= 60; minute += 5) {
        series.push(computeSnapshot(satellites, observer, new Date(start.getTime() + minute * 60_000)).above25);
    }
    return series;
}
export function subpoint(satellite, at = new Date()) {
    const state = propagate(satellite.satrec, at);
    if (!state || typeof state.position === "boolean")
        return null;
