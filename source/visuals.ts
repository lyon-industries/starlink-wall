...
export function skyPlot(satellites: VisibleSatellite[], width = 30, height = 12): string[] {
  ...
  const plot = (x: number, y: number) => {
    ...
    if (px < 0 || py < 0 || px >= pixelsWide || py >= pixelsHigh) return
  ...
  return cells.map((row) =>
    row.map((value) => (value === 0 ? " " : String.fromCodePoint(0x2800 + value))).join(""),
  )
}
...
export function nullableSparkline(values: Array<number | null>): string {
  const present = values.filter((value): value is number => value !== null)
  if (present.length === 0) return values.map(() => "×").join("")
  const bars = "▁▂▃▄▅▆▇█"
