...
export async function measureNetworkSample(checkDish = false): Promise<NetworkSample> {
...
export function summarizeNetwork(samples: NetworkSample[], intervalSeconds = 5): NetworkSummary {
  if (samples.length === 0) {
    return {
      status: "STARTING",
      currentMs: null,
  ...
  if (samples.slice(-3).every((sample) => sample.rttMs === null)) status = "OFFLINE"
  else if (currentMs === null) status = "DEGRADED"
  else if (lossPercent >= 10) status = "DEGRADED"
  else if (successful.length < 12) status = "LEARNING"
  else if ((successful.length >= 20 && (p95Ms ?? 0) >= 250) || (jitterMs ?? 0) >= 80) status = "DEGRADED"
  else if (lossPercent > 0 || (successful.length >= 20 && (p95Ms ?? 0) >= 140) || (jitterMs ?? 0) >= 35) status = "WATCH"
  else status = "ONLINE"

  return { status, currentMs, medianMs, p95Ms, jitterMs, lossPercent, samples: samples.length, outageSeconds }
}

async function measureHttpsRtt(): Promise<number | null> {
...
async function detectDish(): Promise<boolean> {
  try {
